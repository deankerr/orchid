...
const globalAverageScore =
  (await aggregateScoreByUser.sum(ctx)) / (await aggregateScoreByUser.count(ctx))
```
...
const avgScoreForUser =
  (await aggregateScoreByUser.sum(ctx, forUser)) / (await aggregateScoreByUser.count(ctx, forUser))
```
...
through all documents. But sometimes you want big-picture data that encompasses
...
// convex/schema.ts
defineSchema({
  photos: defineTable({ album: v.string(), url: v.string() }).index('by_album_creation_time', [
    'album',
  ]),
})
```
...
// have the same key.
const aggregate = new DirectAggregate<{ Key: number; Id: string }>(components.aggregate)
