...
export const insertSnapshot = internalMutation({
  ...
  handler: async (ctx, { category, key, batchTimestamp, data }) => {
    ...
    await ctx.db.insert('snapshots_old', {
      category,
      key,
      batchTimestamp,
      hash,
    ...
    console.log(
      '[snapshot]',
      'original: ',
      prettyBytes(json.length),
      'compressed:',
      prettyBytes(compressed.length),
    )
  },
})
...
export const deleteAllSnapshots = internalMutation({
  handler: async (ctx) => {
    const snapshots = await ctx.db.query('snapshots_old').collect()
...
// temp. testing our assumptions about the data
